...
void collectBonuses(const InputData &data, Solution &solution) {
    ...
    for (int city : solution.tour) {
        if (data.b[city] > 0) {
            solution.collectedBonuses.push_back(city);
            solution.obj2 += data.t[city];
            solution.bonusSum += data.b[city];
        }
    }
}
...
int calculateTravelTime(const InputData &data, const Solution &solution, int origin, int destination) {
    ...
    for (int i = 0; i < tuorLength; i++) {
        ...
        if (inRoute) {
            if (originBonus) {
                travelTime += data.t[currentCity];
                originBonus = false;
            }

            travelTime += data.y[currentCity][nextCity];
            travelTime += data.t[nextCity];
    ...
    try {
        ...
        while(!stop) {
            ...
            if (model.get(GRB_IntAttr_Status) == GRB_OPTIMAL && !cb.stop_optimization) {
                ...
                cout << "Optimization stopped due to timeout. Solution ignored." << endl;
                stop = cb.stop_optimization;
            } else if (model.get(GRB_IntAttr_Status) == GRB_INFEASIBLE) {
                cout << "GRB_INFEASIBLE" << endl;
                stop = true;
            }
        }
...
int main() {
    ...
    vector<string> instances = {
        ...
        for(string instance : instances)
        {
            ...
            string path_output = "../../resultados/naive_v1/a3/"+outputFileType+"/";
