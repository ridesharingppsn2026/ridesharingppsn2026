...
vector<vector<double>> generateWeightVectors(int NUMSUM||ISSUES) {
  vector<vector<double>> lambda(NUMSUM||ISSUES, vector<double>(3, 0.0f));
  int  s= 23; // dont change
	int cont =0;
	double l1, l2, l3;
    for (int i=0; i<=s && cont<NUMSUM||ISSUES; i++){
        l1 = i;
        for (int j=0; j<=s-i && cont<NUMSUM||ISSUES; j++){
            l2 = j;
            l3 = s-i - l2;
